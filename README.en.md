...
> 版本提醒：本頁為舊版翻譯，尚未同步最新功能。請以 [繁體中文文件](README.md)、[連線與重試](RETRY_POLICY.md) 及 [對話配對與更新恢復](TURN_BINDING.md) 為準。現行版本優先維持原配對，僅在容量失敗、未送出內容、歷史完整且未強制指定來源時允許備援；不會任意刪除前文參照換帳號。用量 API、回應暫存、帳單、自選重設與更新保存功能請參閱繁體中文文件。
