...
帳號用量查詢依自己的週期執行，不會因一般請求持續回傳標頭而一直延後。API 回傳有效資料時優先採用；查詢失敗時使用較新的有效標頭備援，兩者皆無新資料則保留舊值。上游百分比仍可能延遲或以較粗的刻度更新，這不是逐次請求的即時計費表。
...
- **容量拒絕與節流分開處理**：尚未送出內容的容量拒絕退回代理跨重連額度，每輪嘗試上限與上游冷卻仍保留。已允許容量備援時，不另加代理的 30 秒冷卻。
- **冷卻後有限探測**：其他嘗試耗盡額度時暫停 30 秒，再給一次恢復探測。串流以助理訊息交付等待原因並結束，非串流回傳 HTTP 429；真正的上游故障仍回報失敗。
