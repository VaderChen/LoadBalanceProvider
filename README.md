...
- **避免半套工具呼叫**：Codex 串流 Responses 先暫存單筆回應，確認成功完成後才送出，因此不逐字顯示；超過 2 MiB 暫存上限時明確停止，不改送部分內容。
...
- **冷卻後有限探測**：嘗試額度用盡後暫停 30 秒，再給一次恢復探測，仍遵守上游冷卻；不重新發放整套額度。串流透過失敗終止事件回報，非串流回傳 HTTP 429 與等待提示。
