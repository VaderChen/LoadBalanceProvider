...
新版配對資料預設保存在 `data/provider_routes.db`；若自訂進階設定檔位置，則放在該檔案所在目錄。首次使用會匯入仍有效的舊配對並保留舊 JSON，但後續配對不再同步寫入舊檔。備份或搬移時請停止服務並保留完整資料目錄；降版須還原與舊版相配的完整備份，不能只靠舊 JSON 恢復升級後的配對。

更新後先確認原 Provider、認證及資料路徑仍正確，再查看各帳號用量查詢狀態。已結束的錯誤回應不會因更新自動接續；需要重新提出請求，涉及工具操作時先確認執行結果。更新與重啟不能消除上游過載。
