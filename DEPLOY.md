...
| `provider_retry_wait_seconds` | 30 | 首次排隊、失敗後退避各自的累計上限，可設 0–300 秒；0 不等待。重連入口等待代理冷卻另有同值的單次上限。預設一般請求最多 60 秒等待，遇到入口冷卻時最多 90 秒，均不含上游執行時間。 |
...
- 重連遇到代理額度冷卻或探測窗口限制時，在等待上限內保留連線，到期後自動嘗試接續；等待不占上游併發名額，也不解除原來源綁定或上游冷卻。非串流請求等待期間沒有 SSE 心跳，需另留意用戶端與反向代理逾時。
- 無法繼續等待時，尚未開始串流則回 HTTP 429 與 `Retry-After`；已開始 SSE 則使用失敗終止事件，不再把節流原因當成已完成的助理回應。真正的上游 Responses 故障也維持失敗語意；只有工具防重播封鎖仍以完成型助理通知提醒確認工具結果，不代表原任務成功。
- 用戶端取消或下游寫入失敗時會停止等待，不應透過無限制增加逾時來掩蓋問題。請區分上游拒絕、過載、等待標頭逾時與真正的網路斷線。
