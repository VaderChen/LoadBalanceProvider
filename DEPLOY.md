...
| `retry_count` | 尚未轉送有效內容時允許的額外重試次數；容量備援也共用此上限，另受固定來源重試上限限制。文字或工具參數已送出後不再重播。 |
...
Codex OAuth 的背景用量更新改用帳號用量 API，不發送模型推論請求；一般回應標頭保留為備援。串流 Responses 的內容在確認單筆回應成功後才送出，等待期間包含心跳，反向代理需允許串流並關閉回應緩衝。部署前請確認 [串流連線與反向代理](#串流連線與反向代理) 設定。
